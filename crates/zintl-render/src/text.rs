...
use crate::scaling::{
    ...
    LogicalPoint, LogicalRect, LogicalScale, ScaleFactor,
};
...
impl Font {
    pub fn new(
        ...
        let scale_device = scale.in_device_pixels(&scale_factor);
        let atlas = Atlas::new(scale_device.max(128), scale_device.max(32));
        let scaled = ab_font.as_scaled(scale_device as f32);
        let height = scaled.height().in_logical_pixels(&scale_factor);
        let line_gap = scaled.line_gap().in_logical_pixels(&scale_factor);
        let ascent = scaled.ascent().in_logical_pixels(&scale_factor);
        let descent = scaled.descent().in_logical_pixels(&scale_factor);
    ...
    pub fn get_glyph(&self, c: char) -> Glyph {
        ...
        let scaled = self.ab_font.as_scaled(self.scale_device as f32);
        ...
        let rect = {
            if let Some(g) = self.ab_font.outline_glyph(g) {
                ...
                GlyphRect {
                    width: advance.in_logical_pixels(&self.scale_factor),
                    height: self.height,
                    bounds: LogicalRect {
                        min: LogicalPoint::new(
                            px_bounds.min.x.in_logical_pixels(&self.scale_factor),
                            px_bounds.min.y.in_logical_pixels(&self.scale_factor),
                        ),
                        max: LogicalPoint::new(
                            px_bounds.max.x.in_logical_pixels(&self.scale_factor),
                            px_bounds.max.y.in_logical_pixels(&self.scale_factor),
                        ),
                    },
                    texture_bounds,
                }
